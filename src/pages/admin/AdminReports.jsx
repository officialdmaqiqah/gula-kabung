...
import { FileText, Calendar, Download, Loader2, PieChart, TrendingUp, Wallet, ShieldCheck, Briefcase, Lock, CheckCircle2, AlertCircle, Clock, ArrowRight } from 'lucide-react';
...
export default function AdminReports() {
  ...
  const cashflow = useMemo(() => {
    ...
    // 2. Financing Activities (Take from Investors table to match Neraca)
    // For "All Time", this matches totalCapital. For filtered, we should ideally track date of investment.
    // Assuming Modal Awal category in incomes is the cash movement.
    const financing = filteredIncomes.filter(i => i.kategori === 'Modal Awal').reduce((sum, i) => sum + Number(i.jumlah), 0) || 
                      (filterPeriod === 'Semua' ? investors.reduce((sum, inv) => sum + Number(inv.modal), 0) : 0);
    
    const netChange = operational + financing;
    
    // 3. Beginning Balance (Sum of all account initial balances)
    const beginningBalance = accounts.reduce((sum, acc) => sum + (Number(acc.saldo_awal) || 0), 0);

    return { opIn, opOut, operational, financing, netChange, beginningBalance };
          ...
          <div className="space-y-12 max-w-3xl">
            <div>
              ...
              <div className="space-y-4 px-6 border-l-2 border-brand-brown/5">
                <div className="flex justify-between items-center"><span className="text-sm font-bold text-brand-brown/60">Penerimaan dari Pelanggan</span><span className="text-sm font-black text-emerald-600">+{formatRupiah(cashflow.opIn)}</span></div>
                <div className="flex justify-between items-center"><span className="text-sm font-bold text-brand-brown/60">Pembayaran Beban & Stok</span><span className="text-sm font-black text-rose-500">-({formatRupiah(cashflow.opOut)})</span></div>
            ...
            <div className="pt-10 border-t-2 border-brand-brown/10 space-y-4">
               <div className="flex justify-between items-center px-6"><span className="text-sm font-bold text-brand-brown/40 uppercase tracking-widest">Saldo Awal Kas</span><span className="text-sm font-bold text-brand-brown">{formatRupiah(cashflow.beginningBalance)}</span></div>
               <div className="flex justify-between items-center px-6"><span className="text-sm font-bold text-brand-brown/40 uppercase tracking-widest">Kenaikan/Penurunan Neto</span><span className={`text-sm font-bold ${cashflow.netChange >= 0 ? 'text-emerald-600' : 'text-rose-600'}`}>{formatRupiah(cashflow.netChange)}</span></div>
               <div className="flex justify-between items-center p-8 bg-brand-gold rounded-3xl text-brand-brown shadow-xl">
                 <div><span className="text-[10px] font-black uppercase tracking-[0.4em] opacity-60">Saldo Akhir Kas</span><h4 className="text-4xl font-black tracking-tighter mt-1">{formatRupiah(cashflow.beginningBalance + cashflow.netChange)}</h4></div>
                 <Wallet className="w-12 h-12 text-brand-brown opacity-40" />
               </div>
            </div>
          </div>
        </div>
