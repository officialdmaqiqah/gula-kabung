...
export default function AdminDashboard() {
  ...
  const fetchDashboardData = async () => {
    try {
      ...
      const { data: receivingData } = await supabase.from('kabung_receiving').select('*');
  ...
  const {
    ...
    receiving.forEach(r => {
      if (r.tanggal_terima && r.tanggal_terima.startsWith(thisMonth)) {
  ...
  const accountBalances = useMemo(() => {
    ...
    mutations.forEach(m => {
      if (balances[m.dari_rekening_id] !== undefined) balances[m.dari_rekening_id] -= Number(m.jumlah);
      if (balances[m.ke_rekening_id] !== undefined) balances[m.ke_rekening_id] += Number(m.jumlah);
    });
          ...
          <div className="flex-grow space-y-6">
            <div className="p-6 bg-brand-brown/[0.02] rounded-3xl border border-brand-brown/5 flex justify-between items-center">
              <div>
                ...
                <h4 className="text-3xl font-black text-brand-brown">{totalPcsBalance} <span className="text-sm font-bold text-brand-brown/30">pcs</span></h4>
              </div>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="p-5 bg-emerald-50 rounded-3xl border border-emerald-100">
                ...
                <h4 className="text-2xl font-black text-emerald-700">{totalPcsIn} <span className="text-xs font-bold text-emerald-600/40 italic">pcs</span></h4>
              </div>
              <div className="p-5 bg-rose-50 rounded-3xl border border-rose-100">
                ...
                <h4 className="text-2xl font-black text-rose-700">{totalPcsOut} <span className="text-xs font-bold text-rose-600/40 italic">pcs</span></h4>
              </div>
            </div>
          </div>
